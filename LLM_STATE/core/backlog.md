...
### QEMURunner swtpm socket path exceeds macOS sun_path limit
...
`QEMURunner.start` composes the swtpm control socket as
...
Resolved 2026-04-20 by `fix(qemu): stage swtpm+monitor sockets under $TMPDIR (sun_path limit)`
...
### Disable SSH in macOS and Linux golden images
...
Unblocked after Task 3 P5 landed 2026-04-19.

The macOS and Linux golden images ship with SSH enabled for the
...
### recordingTask cancel-sleep pattern is a latent SIGABRT
...
`recordingTask` in `TestAnywareServer` cancels a sleeping `Task` from
...
### `input click --window` reports VNC click ~40px below target on Tahoe
...
The `--window` option translates window-relative coordinates to
...
### Consolidate ConnectionSpec.namedSpecPath with VMPaths
...
`ConnectionSpec.namedSpecPath(for:)` and `VMPaths` both compute
...
### Audit testanyware-agent entitlements for Documents folder access on Tahoe
...
During Note Editor VM verification on 2026-04-18, running
...
### Deduplicate UnifiedRole.swift across CLI and agent targets
...
`UnifiedRole.swift` exists in both the CLI target and the agent-side
...
### Menu bar drill-down: snapshot only surfaces top-level items
...
`testanyware agent snapshot --window "Menu Bar"` only returns
...
### VirtioFS serves stale cached files after host update
...
VirtioFS shared filesystem can serve cached (stale) file content
