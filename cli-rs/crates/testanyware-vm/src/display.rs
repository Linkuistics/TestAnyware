...
/// Best-effort macOS guest resolution switch over the agent (ADR-0014).
///
/// Host-compiles [`SET_DISPLAY_MODE_SWIFT`], uploads it to the guest over
/// `/upload`, and `/exec`s it with the px `target`. **Tolerant throughout**:
/// the resolution switch rides the same optionally-degraded contract as the
/// agent endpoint itself (ADR-0013/0014) — a missing `swiftc`, a compile /
/// upload / exec failure, or a helper that finds no matching 1× mode all warn
/// and return, leaving the VM started at its current resolution. It never fails
/// `vm start`.
///
...
pub async fn apply(host: &str, port: u16, target: (u32, u32), paths: &VmPaths, id: &str) {
    let (w, h) = target;
    ...
    let outcome = switch_over_agent(host, port, &bin, w, h).await;
...
async fn switch_over_agent(
    ...
    let command = format!("chmod +x {remote} && {remote} {w} {h}; rc=$?; rm -f {remote}; exit $rc");
    let result = client
